...
You don't need to be technical to contribute. Find the talk text, fill in a short form on the website, paste the transcript, and submit. This guide tells you exactly where to find talks and how to record them accurately.

**Your goal for each talk:**
1. Find the primary source (scan, webpage, etc.)
2. Record the speaker, date, source, and fidelity information in the form
3. Paste the full transcript text
4. Submit — an admin reviews before anything goes live
...
2. **Add talks one by one.** Click "View Session" → "Add Talk". Fill in the form for each talk in that session.
3. **Save drafts as you go.** Each talk saves as a draft — you can close the browser and come back later.
4. **Submit when finished.** Once you've entered all the talks for that session, click "Submit for Review." You won't be able to edit after this point.
5. **Wait for review.** An admin will review each talk. If anything needs fixing, it'll be returned to you with notes and you can re-edit.

### Quality Checklist

Before submitting, verify each talk has:
- [ ] Speaker name spelled correctly (use the standard spelling from ChurchofJesusChrist.org)
- [ ] Source title filled in (e.g., "Journal of Discourses, Vol. 1")
- [ ] Source URL is a working link — click it to confirm
- [ ] Source type and fidelity set correctly (descriptions are in the dropdowns)
- [ ] Transcript pasted in full, with paragraph breaks preserved
- [ ] No added commentary, headers, or formatting beyond paragraph breaks
- [ ] Uncertain readings marked `[?]`, illegible passages marked `[illegible]`
- [ ] Any problems or gaps noted in the Notes field
